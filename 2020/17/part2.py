...
class Grid(object):
    ...
    def __init__(self, slice):
        ...
        for s, value in self.slices.items():
            for w in range(len(value)):
                for r in range(len(self.slices[s][w])):
                    for c in range(len(self.slices[s][w][r])):
                        self.get_neighbours((s, w, r, c))
                        if self.slices[s][w][r][c] == '#':
                            self.cubes[(s, w, r, c)] = Grid.Cube(active=True)
                        elif self.slices[s][w][r][c] == '.':
                            self.cubes[(s, w, r, c)] = Grid.Cube(active=False)
    ...
    def get_neighbours(self, pos):
        ...
        rows = [r-1, r, r+1]
        cols = [c-1, c, c+1]
        positions = list(itertools.product(slices, wdim, rows, cols))
        positions.remove((s, w, r, c))
